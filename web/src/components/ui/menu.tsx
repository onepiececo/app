...
export function MenuTrigger({
  ...
  const trigger = render ?? <Button size={size} variant={variant} />;
  ...
  return (
    <MenuPortal {...portalProps}>
      ...
      >
        <MenuPrimitive.Popup
          className={cn(
            "relative flex not-[class*='w-']:min-w-36 origin-(--transform-origin) rounded-lg bg-white text-zinc-950 outline-none shadow-[0_0_0_1px_var(--color-zinc-300),0_8px_18px_rgb(0_0_0/0.12)] inset-shadow-[0_1px_0_rgb(255_255_255/0.75)] dark:bg-zinc-900 dark:text-white dark:shadow-[0_0_0_1px_rgb(255_255_255/0.14),0_8px_18px_rgb(0_0_0/0.45)] dark:inset-shadow-[0_1px_0_rgb(255_255_255/0.08)]",
            "transition-[scale,opacity] duration-[160ms] ease-out data-ending-style:duration-[140ms] data-starting-style:scale-95 data-starting-style:opacity-0 data-ending-style:scale-95 data-ending-style:opacity-0 data-instant:transition-none",
            "focus:outline-none",
            className,
          )}
...
export function MenuItem({
  ...
  return (
    <MenuPrimitive.Item
      className={cn(
        "relative flex min-h-8 cursor-default select-none items-center gap-2 rounded-sm px-2.5 py-1.5 text-base text-foreground outline-none transition-colors data-disabled:pointer-events-none data-highlighted:bg-accent data-highlighted:text-accent-foreground data-inset:ps-8 data-disabled:opacity-64 data-[variant=destructive]:text-destructive data-[variant=destructive]:data-highlighted:bg-destructive/10 data-[variant=destructive]:data-highlighted:text-destructive sm:text-sm [&>svg:not([class*='opacity-'])]:opacity-80 [&>svg:not([class*='size-'])]:size-4 [&>svg]:pointer-events-none [&>svg]:-mx-0.5 [&>svg]:shrink-0",
        className,
      )}
      data-inset={inset}
      data-slot="menu-item"
    ...
    >
      ...
      {variant === "switch" ? (
        <MenuPrimitive.CheckboxItemIndicator
          className="inline-flex h-[calc(var(--thumb-size)+2px)] w-[calc(var(--thumb-size)*2-2px)] shrink-0 items-center rounded-full p-px outline-none transition-[background-color,box-shadow] duration-[120ms] ease-out [--thumb-size:--spacing(3.5)] data-unchecked:bg-zinc-200 data-unchecked:ring-1 data-unchecked:ring-black/5 data-unchecked:ring-inset data-checked:bg-zinc-900 data-checked:ring-1 data-checked:ring-zinc-950 data-checked:shadow-[0_1px_2px_rgb(0_0_0/0.18)] data-disabled:opacity-64 dark:data-unchecked:bg-white/5 dark:data-unchecked:ring-white/15 dark:data-checked:bg-white/25 dark:data-checked:ring-transparent dark:data-checked:shadow-[0_1px_2px_rgb(0_0_0/0.35)]"
          keepMounted
        >
