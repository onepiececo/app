...
export const chartVariants = tv({
  slots: {
    ...
    tooltipItems: "grid gap-1.5",
    tooltipItem:
      "[&>svg]:text-muted-foreground flex w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5",
    tooltipIndicator:
      "shrink-0 rounded-xs border-(--color-border) bg-(--color-bg)",
    tooltipValueRow: "flex flex-1 justify-between leading-none",
    tooltipNameGroup: "grid gap-1.5",
    tooltipName: "text-muted-foreground",
    tooltipValue: "text-foreground font-mono font-medium tabular-nums",
    legend: "flex items-center justify-center gap-4",
    legendItem:
      "[&>svg]:text-muted-foreground flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3",
    legendIcon: "h-2 w-2 shrink-0 rounded-xs",
  },
  variants: {
    indicator: {
      dot: {
        tooltipIndicator: "h-2.5 w-2.5",
        tooltipItem: "items-center",
      },
      line: {
    ...
    nested: {
      true: {
        tooltipIndicator: "my-0.5",
        tooltipValueRow: "items-end",
      },
      false: {
        tooltipValueRow: "items-center",
      },
    },
    verticalAlign: {
      top: {
  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        data-slot="chart"
        data-chart={chartId}
        className={chartVariants().container({ class: className })}
        {...rest}
      >
...
export const ChartTooltipContent = ({
  ...
  const styles = chartVariants({ indicator });
  const tooltipLabel = (() => {
    ...
    if (labelFormatter) {
      return (
        <div className={styles.tooltipLabel({ class: labelClassName })}>
          {labelFormatter(value, payload)}
        </div>
      );
    }
    if (!value) return null;
    return <div className={styles.tooltipLabel({ class: labelClassName })}>{value}</div>;
  })();
  if (!active || !payload?.length) return null;
  const nestLabel = payload.length === 1 && indicator !== "dot";
  return (
    <div
      className={styles.tooltip({ class: className })}
    >
            ...
            >
              {formatter && item?.value !== undefined && item.name ? (
                ...
                <>
                  ...
                  <div
                    className={chartVariants({ nested: nestLabel }).tooltipValueRow()}
                  >
                    <div className={styles.tooltipNameGroup()}>
                      {nestLabel ? tooltipLabel : null}
...
export const ChartLegendContent = ({
  ...
  const { config } = useChart();
  const styles = chartVariants({ verticalAlign });
  if (!payload?.length) return null;
  return (
    <div
      className={styles.legend({ class: className })}
    >
      {payload.map((item) => {
